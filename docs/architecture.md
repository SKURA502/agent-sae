...
- Stage 2 使用 lr=5e-4 配合早停，防止在小规模有标签集上过拟合
...
详细决策依据见 [`datasets.md`](../datasets.md)。三类角色不可混用：

| 角色 | 目的 | 标签要求 | 规模 |
|------|------|---------|------|
| **SAE 训练** | 学习残差流表征 | 无（Stage 1）/ CALL/NO_CALL（Stage 2） | 50M+ tokens / ~6K 样本 |
| **特征发现** | 通过 CALL/NO_CALL 差值定位门控 features | 严格二元标签对 | ~6.6K 样本 |
| **实验评测** | 验证 H1/H2/H3 | 因假设而异 | 测试集规模 |

### SAE 训练数据集

- **Stage 1** — OpenWebText2（50–100M tokens，无标签）
- **Stage 2 可选：域适应** — When2Call SFT 15K + BFCL simple/parallel/multiple（无标签）；tau2-bench 无现成轨迹，不可直接用
- **Stage 2 特征专项** — When2Call Pref 平衡采样（3K CALL + 3K NO_CALL）

### 特征发现数据集

用于计算 `E[f|CALL] − E[f|NO_CALL]`、AUROC 及线性探针：

- **When2Call Pref**：3K CALL + 3K NO_CALL（从 9K Pref 集 1:1 采样）——主要来源
- **BFCL Irrelevance**：1,124 条额外 NO_CALL（`BFCL_v4_irrelevance` + `live_irrelevance`）
- **BFCL Simple**：~240 条分布外 CALL 样本（`BFCL_v4_simple_python` + `live_simple`）——补充 CALL 侧多样性
...
| 假设 | 数据集 | 规模 | 说明 |
|------|--------|------|------|
| **H1** — 门控可分离性（主测） | When2Call MCQ 二类子集 | 2,590 | 仅 tool_call vs cannot_answer；排除 request_for_info |
| **H1** — 泛化验证 | BFCL Irrel 1,124 + BFCL Simple ~240 | ~1.4K | live_relevance 仅 16 条，不可单独用 |
| **H2** — 证据积累 | sandbox rollouts（tau2-bench tasks 作 prompt 种子） | 100–500 episodes | 动态多步；tau2-bench 提供业务语境 |
| **H3** — 因果 steering | When2Call MCQ 二类子集（同 H1） | 2,590 | 排除 request_for_info 保持翻转率定义干净 |

H2 依赖 agent loop 完整跑通。MCQ 中的 request_for_info（1,062 条）可在论文中作为 out-of-distribution 探针单独分析。
