...
| Stage 2 | When2Call Pref（有标签） | ~24k样本 | 专项学习工具调用决策特征 |
...
| 角色 | 目的 | 标签要求 | 规模 |
|------|------|---------|------|
| **SAE 训练** | 学习残差流表征 | 无（Stage 1）/ CALL/NO_CALL（Stage 2） | 50M+ tokens / ~24K 样本 |
| **特征发现** | 通过 CALL/NO_CALL 差值定位门控 features | 严格二元标签对 | ~9K 样本 |
| **实验评测** | 验证 H1/H2/H3 | 因假设而异 | 测试集规模 |

### SAE 训练数据集

- **Stage 1** — OpenWebText2（50–100M tokens，无标签）
- **Stage 2** — When2Call Pref（9K）+ When2Call SFT（15K）；排除整个 `when2call_test_mcq.jsonl`（含 request_for_info）

### 特征发现数据集

用于计算 `E[f|CALL] − E[f|NO_CALL]`、AUROC 及线性探针：

- **When2Call Pref 全量**：3K CALL + 6K NO_CALL，分类别计算条件期望，无需人工平衡采样

注意：`when2call_train_sft.jsonl`（15K）全为 NO_CALL，不参与特征发现差值计算。

### 实验评测数据集

| 假设 | 数据集 | 规模 | 说明 |
|------|--------|------|------|
| **H1** — 门控可分离性 | When2Call MCQ 二类子集 | 2,590 | 仅 tool_call vs cannot_answer；排除 request_for_info；完全未见数据 |
